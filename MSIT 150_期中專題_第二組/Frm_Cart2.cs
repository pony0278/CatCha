...
namespace Cart2
{
    public partial class Frm_Cart2 : Form
    {
        ...
        {
            InitializeComponent();
            this.BackColor = ColorTranslator.FromHtml("#FEF5EF");
        ...
        private void CartDetailsPage_Load(object sender, EventArgs e)
        {
            ...
            foreach (var i in listCartDetails)
            {
                ...
                this.labCartTotal.Text = $"商品小計 {CarttoBill.CartTotal:c2}";
            ...
            else
            {
                CarttoBill.Points = points;
                CarttoBill.PointsValue = PointsExchangeMoney(CarttoBill.Points);
                this.ckbUseLoyaltyPoints.Text = $"使用 {CarttoBill.PointsValue * 1000} 點紅利點數折抵 {CarttoBill.PointsValue:c2}";
        ...
        private void btnSendOrder_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                frmCart3.Show();
                //把庫存扣掉
                var quntities = from p in dbContext.Shop_Product_Total
                                from od in p.Shop_Order_Detail_Table
                                where od.Shop_Order_Total_Table.Member_ID == memberID && od.Shop_Order_Total_Table.Order_Status_ID == 1
                                select new { 商品ID = od.Product_ID, 商品數量 = od.Product_Quantity, 商品庫存 = p.Remaining_Quantity };
                var q = quntities.ToList();
                foreach (var p in q)
                {
                    int rq = (int)(p.商品庫存 - p.商品數量);

                    //更新商品庫存
                    var pTotal = dbContext.Shop_Product_Total.FirstOrDefault(pt => pt.Remaining_Quantity == p.商品庫存);
                    if (pTotal != null)
                    {
                        pTotal.Remaining_Quantity = rq;
                    }
